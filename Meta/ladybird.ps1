...
if ([string]::IsNullOrEmpty($command) -or ($null -ne $other_args -and $other_args.Contains("help")))
{
    Show-Help
    exit 0
}

if (-not (Get-UserIsAdmin))
{
    Write-Host "Do not run $(Get-ScriptName) with an admin user; your Build directory will be owned by the admin user!"
    exit 0
}
